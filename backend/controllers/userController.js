...
exports.registerUser = catchAsyncError(async (req, res, next) => {
  ...
  await createAuditLog(user._id, 'User Registered');
...
exports.loginUser = catchAsyncError(async (req, res, next) => {
  ...
  console.log(user.accountLocked)
  ...
  if(isPasswordMatched && !user.accountLocked){
    user.failedLoginAttempts = 0;
  ...
  //});
  //----> Above code same work as below (below code comes from token & cookie seprate file function)
  await createAuditLog(user._id, 'User Logged In');
...
exports.forgotPassword = catchAsyncError(async (req, res, next) => {
  ...
  const resetPasswordUrl = `localhost:3000/password/reset/${resetToken}`; // its because our server run on frontend 3000 and it different from backend ,,,,till the hosting we use this way
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: `Casm Ecommerce Password Recovery`,
      message,
    });
    res.status(200).json({
      success: true,
